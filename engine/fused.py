...
def _fp8_quant_kernel(x_ptr, o_ptr, inv_ptr, n, BLOCK: tl.constexpr):
    # tensorwise fp8 cast: o = (x * (448/amax)) -> e4m3. One read of x + one fp8 write, replacing
    # the naive torch abs/amax/div/cast chain (~45ms/prefill) with ~3ms. amax computed by the caller.
    pid = tl.program_id(0)
...
def quant_fp8_tensorwise(x):
    """x contiguous bf16 [M,K] -> (xf e4m3 [M,K], scale [1,1] fp32) with one fused cast kernel."""
