...
**Scoring model (exact, `tests/score_model.py`):** `official_score = 506.522 / geomean(p50 seconds over the 6 private workloads)` (`score*metricMs/1000` is 506.52223 on every run), and `geomean(batch*output_tokens)` over the private set is 506.5 vs 203 for the public set, so the private workloads are heavier than the public ones. `log(score) = -0.266 + 0.142*log(B1 512->32 tok/s) + 0.449*log(B4 2048->32) + 0.444*log(B16 512->128)` fits all 11 runs to <0.8%. So 1% on B1 is worth ~0.14% of score and 1% on B4-2048 or B16-512 ~0.45%: **do not tune B1**. `tests/bench.py` now prints a predicted official score and % of bandwidth roofline (pod numbers run ~0.7% above the platform's). Leaders (Sep 20): SSS 1144, Silver Bullet 1138, dryfter 1137.
